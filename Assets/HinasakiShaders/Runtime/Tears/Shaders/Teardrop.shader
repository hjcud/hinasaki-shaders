...
Shader "Hinasaki/Teardrop"
{
    Properties
    {
        ...
        _TearPosX ("X Position", Range(-0.5, 0.5)) = 0
    ...
    SubShader
    {
		...
		Pass
		{
            ...
            sampler2D _MainTex, _MainMask, _SecondTex, _SecondNoise;
            float4 _MainTex_ST, _MainMask_ST;
            float4 _SecondTex_ST, _SecondNoise_ST;
            float _DropPosX, _Randwidth, _DropWaveFreq, _DropWaveWidth;
            float _TearPosX, _TearPosY, _NoiseSize, _NoiseSpeed, _NoisePower, _TexPower, _RefPower, _DropSpeed, _gridModeToggle;
			...
			fixed4 frag (v2f i) : SV_Target
			{
                ...
                float2 colUV = screenUV + (((mainTexUV + (secondTexUV * (secondNoiseTexUV * _NoisePower))) * (_RefPower * 0.1)) * mainMaskUV);
